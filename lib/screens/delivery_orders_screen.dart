...
class _DeliveryOrdersScreenState extends State<DeliveryOrdersScreen> {
  ...
  void loadData() {
    if (_isInit && mounted) {
      ...
      try {
        Provider.of<DataProvider>(context).fetchDeliveryOrders(_driver).then((orders) {
          setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    deliveryOrders = Provider.of<DataProvider>(context).deliveryOrders;
    deliveryOrders.sort((order1, order2) => order2.orderTime.compareTo(order1.orderTime));
    return Scaffold(
      body: Stack(
        children: [
          Container(
                ...
                child: _isLoading
                    ...
                    : (deliveryOrders.length > 0)
                        ? SingleChildScrollView(
                            child: Column(children: deliveryOrders.map((order) => _orderItem(order)).toList()))
                        : Center(
                            child: Text(
                              'No Orders.',
