...
import 'checkboxOrientation.dart';
...
class _GroupedCheckboxState extends State<GroupedCheckbox> {
  ...
  Widget generateItems() {
    ...
    if (widget.orientation == CheckboxOrientation.VERTICAL) {
      ...
      finalWidget = SingleChildScrollView(scrollDirection: Axis.vertical, child: Column(children: content));
    } else if (widget.orientation == CheckboxOrientation.HORIZONTAL) {
      for (final item in widgetList) {
        content.add(Column(children: <Widget>[item]));
      }
      finalWidget = SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: content));
    } else {
    ...
    return Row(
      ...
      children: <Widget>[
        Checkbox(
            ...
            onChanged: (widget.disabled != null && widget.disabled.contains(widget.itemList.elementAt(index)))
                ? null
                : (bool selected) {
                    selected ? selectedListItems.add(widget.itemList[index]) : selectedListItems.remove(widget.itemList[index]);
                    setState(() {
                      widget.onChanged(selectedListItems);
                    });
                  }),
        Expanded(
            child: Text(widget.itemList[index].isEmpty ? '' : widget.itemList[index],
                style: widget.disabled != null && widget.disabled.contains(widget.itemList.elementAt(index))
